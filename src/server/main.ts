...
import { SoftwareFactoryEngine } from '../engine/main'
...
fastify.post('/api/selected-theme', async (req: any) => {
    ...
    fastify.post('/api/engine/process/:stageId', async (request, reply) => {
        try {
            const { stageId } = request.params as any;
    ...
    fastify.post('/api/engine/package', async (request, reply) => {
        try {
            const { files, projectName } = request.body as any;
...
fastify.post('/api/stakeholders', async (req) => { db.stakeholders.push({ id: `SH-${Date.now()}`, ...req.body as any }); return { success: true } });
fastify.delete('/api/stakeholders/:id', async (req: any) => { db.stakeholders = db.stakeholders.filter((s: any) => s.id !== req.params.id); return { success: true } });

fastify.get('/api/elicitation', async () => db.elicitation);
fastify.post('/api/elicitation', async (req) => { db.elicitation.push({ id: `ACT-${Date.now()}`, ...req.body as any }); return { success: true } });
fastify.delete('/api/elicitation/:id', async (req: any) => { db.elicitation = db.elicitation.filter((s: any) => s.id !== req.params.id); return { success: true } });

fastify.get('/api/questions', async () => db.questions);
fastify.post('/api/questions', async (req: any) => { 
    const { category, id, answer } = req.body;
    if (db.questions[category]) {
        db.questions[category] = db.questions[category].map((q: any) => q.id === id ? { ...q, answer } : q);
    }
    return { success: true };
});
...
fastify.post('/api/requirements', async (req) => { 
    db.requirements.push({ id: `REQ-${Date.now()}`, status: 'Pendiente', ...req.body as any }); 
    return { success: true };
});
fastify.delete('/api/requirements/:id', async (req: any) => { db.requirements = db.requirements.filter((r: any) => r.id !== req.params.id); return { success: true } });

fastify.get('/api/qa', async () => db.qa.checklist);
fastify.post('/api/qa', async (req: any) => { 
...
fastify.post('/api/selected-stack', async (req: any) => {
...
fastify.post('/api/selected-methodology', async (req: any) => {
...
fastify.get('/api/summary', async () => {
    ...
    if (db.selectedStack.languages && db.selectedStack.languages.length > 0) recommendedTeam.push('Backend Developer');
    if (db.selectedStack.frameworks && db.selectedStack.frameworks.length > 0) recommendedTeam.push('Frontend Developer');
    if (db.qa.checklist['Security']) recommendedTeam.push('Security Specialist');
    if (reqCount > 5) recommendedTeam.push('QA Engineer');
