...
export class SoftwareFactoryEngine {
  constructor() {
  ...
  async processStage(stageId: number, inputData: any, currentProjectState: any): Promise<any> {
    console.log(`[Engine] Processing Stage ${stageId}...`);
    let output: any;
    let logs: string[] = [`Starting Stage ${stageId}...`];
    ...
    try {
        switch (stageId) {
            case 1:
                ...
                if (!currentProjectState.requirements) currentProjectState.requirements = {};
  ...
  processRequirements(data: any): any {
    ...
    let domain = 'General';
    if (text.includes('pago') || text.includes('fintech') || text.includes('banco') || text.includes('dinero')) domain = 'FinTech';
    else if (text.includes('salud') || text.includes('médico') || text.includes('hospital')) domain = 'HealthTech';
    else if (text.includes('tienda') || text.includes('venta') || text.includes('e-commerce')) domain = 'E-Commerce';
    else if (text.includes('educación') || text.includes('curso')) domain = 'EdTech';
  ...
  processDecision(reqData: any, inputData: any = {}): any {
    ...
    return {
      ...
      license: inputData.licenseType || 'Open Source (MIT)'
    };
  }

  // --- STAGE 3: ARCHITECTURE DESIGN ---
  generateArchitecture(decisionData: any, _inputData: any): any {
  ...
  generateDesign(_archData: any, inputData: any): any {
  ...
  // --- STAGE 5: PLANNING ---
  estimateProject(_designData: any, inputData: any): any {
  ...
  async generateCode(projectState: any, inputData: any): Promise<any> {
      ...
      if (backend.toLowerCase().includes('java')) language = 'java';
      else if (backend.toLowerCase().includes('python')) language = 'python';
      ...
      const config: ProjectConfig = {
          ...
          license: projectState.decision?.license?.includes('Closed') ? 'proprietary' : 'mit'
      };
  ...
  // --- STAGE 7: QA & TESTING ---
  runTests(_codeData: any, _inputData: any): any {
  ...
  // --- STAGE 8: DEPLOYMENT ---
  deployArtifact(_qaData: any, inputData: any): any {
  ...
  // --- STAGE 9: MONITORING ---
  configureMonitoring(_deployData: any, inputData: any): any {
  ...
  calculateValuation(_monitorData: any, _inputData: any, fullProjectState: any): any {
      ...
      if (domain === 'FinTech') marketDemand = 1.8;
      if (domain === 'HealthTech') marketDemand = 1.6;
      if (domain === 'E-Commerce') marketDemand = 1.4;
  ...
  packageDelivery(_valuationData: any, _inputData: any): any {
...
function stageRequirements(ctx: any) { 
   ctx.stages.push({ 
     stage: "REQUIREMENTS", 
     output: { 
       validated: true, 
...
function stageDecision(ctx: any) { 
...
function stageArchitecture(ctx: any) { 
   ctx.stages.push({ 
     stage: "ARCHITECTURE", 
     output: { 
   ...
   const archResult = {
       event: 'architecture.ready',
       structure: {
           type: ctx.stages.find((s: any) => s.stage === 'DECISION')?.output.architecture || 'Monolith',
           layers: ['api', 'domain', 'infra']
       }
   };
...
function stagePlanning(ctx: any) { 
   ctx.stages.push({ 
     stage: "PLANNING", 
     output: { 
       sprints: 4, 
...
function stageMonitoring(ctx: any) { 
   ctx.stages.push({ 
     stage: "MONITORING", 
     output: { 
...
function stageValuation(ctx: any) { 
   ctx.stages.push({ 
     stage: "VALUATION", 
     output: { 
       qualityScore: 92, 
...
function stagePackaging(ctx: any) { 
   ctx.stages.push({ 
     stage: "DELIVERY", 
     output: { 
       downloadable: true, 
...
export async function runFactory(requirements: any) { 
   const context: any = { 
     requirements, 
     stages: [], 
   }; 
